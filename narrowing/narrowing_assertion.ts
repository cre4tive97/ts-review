...
function clean(arr :(string|number)[]) :number[] {
    return arr.map(item => {
...
function subject(teacher :{subject : string|string[]}) :string {
    if(typeof teacher.subject === 'string') return teacher.subject;
    else if(Array.isArray(teacher.subject)) return teacher.subject[teacher.subject.length-1];
    else return '없쪄';
}
